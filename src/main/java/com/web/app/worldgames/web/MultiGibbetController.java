...
public class MultiGibbetController {
    private final static Logger log = Logger
	    .getLogger(MultiGibbetController.class);
    ...
    public String home(HttpServletRequest request,
	    HttpServletResponse responsce, Model model) {
    ...
    String onMessage(HttpServletRequest request,
	    ...
	    @RequestParam("data") String data, @RequestParam("word") String word)
	    throws FileNotFoundException {

	User user = (User) request.getSession().getAttribute("user");
	int idGame = (Integer) request.getSession()
		.getAttribute("idGibbetGame");
	...
	if (type.toLowerCase().trim().equals("message")) {
		...
		log.info("Add to opponent word  "
			+ gibbetGame.getHost().getMyWord() + "  "
		...
		log.info("Add to oppanent word  "
			+ gibbetGame.getClient().getMyWord() + "  "
	...
	if (type.toLowerCase().trim().equals("something")) {
	    if (gibbetGame.getClient().getId() == user.getId()
		    && gibbetGame.getClient().getMyWord() != null) {
    ...
    public @ResponseBody
    String ready(HttpServletRequest request, @RequestParam("type") String type,
	    @RequestParam("data") String data, @RequestParam("word") String word) {
	...
	if (type.toLowerCase().trim().equals("update")) {
	    ...
	    if ((gibbetGame.getHost().isReady() && gibbetGame.getClient()
		    .isReady() == false)
		    || (gibbetGame.getHost().isReady() && gibbetGame
			    .getClient() == null)
		    || gibbetGame.getHost().isReady() == false
	...
	if (type.toLowerCase().trim().equals("up")) {
	    if (gibbetGame.getHost().getId() == user.getId()
		    && gibbetGame.getHost().getOpponent() != null) {
		return gibbetGame.getHost().getOpponent();
	    }
	    if (gibbetGame.getClient().getId() == user.getId()
		    && gibbetGame.getClient().getOpponent() != null) {
		return gibbetGame.getClient().getOpponent();
	    }
	}
	if (type.toLowerCase().trim().equals("key")
		&& gibbetGame.getClient().getId() == user.getId()) {
    ...
    public @ResponseBody
    String win(HttpServletRequest request, @RequestParam("type") String type,
