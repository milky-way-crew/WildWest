...
public class RailCard extends SellableCard {
	...
	void effectOnPlayer(Player player) {
		...
		if (cell.getOwner() == null) {
			System.out.println("Press 1 if you want to buy it");
			if (key.equals("1")) {
			...
			if (!owner.equals(player)) {
				...
				System.out
						.println("You are owner of this rail.");
				player.setMoney(player.getMoney());
				player.setPosition(player.getPosition());
			}
		}
	...
	@Override
	void payRentToOwner(Player player, Player owner, int price) {
		player.setMoney(player.getMoney() - price);
		owner.setMoney(owner.getMoney() + price);
	}

	@Override
	int getRent(SellableCard cell, Player player, Player owner) {
		if (isMortage()) {
			player.setMoney(player.getMoney());
			player.setPosition(player.getPosition());
			return 0;
